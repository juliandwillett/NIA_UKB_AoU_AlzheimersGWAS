...
comparePValsBellenguezLoci = function(make_for_testing) {
  ...
  out_df = data.frame(ID = bell_hits_pval$ID,CHR=bell_hits_pval$CHR,Rsid=bell_hits_pval$Rsid,
                      EA=bell_hits_pval$MINOR,Bell_P=bell_hits_pval$Pval_Final,
                      AD_P=NA,UKB_P=NA,AOU_P=NA,AD_by_proxy_P=NA,HWE_ALL=NA,
...
make_files_for_hwe = function() {
  ...
  meta_nia_nimh_hits = vroom("final_meta_results/meta_niagads_nimh_chrposrefalt_p_1e-5.TBL",show_col_types = F) %>%
  ...
  meta_nia_nimh_ukb_aou = vroom("final_meta_results/meta_niagads_nimh_ukb_aou_chrposrefalt_p_1e-5.TBL",show_col_types = F)
  ...
  bed_file = data.frame(CHR = nia_hits$chr,POSS=nia_hits$pos,POSE=nia_hits$pos) %>%
    ...
    add_row(CHR=meta_nia_nimh_ukb_aou$CHR,POSS=meta_nia_nimh_ukb_aou$POS,POSE=meta_nia_nimh_ukb_aou$POS) %>%
    distinct(.keep_all = T)
...
make_all_minor_alleles = function(df,no_dir = F) {
  ...
  for (row in 1:nrow(out_df)) {
    if (out_df$MAF[[row]] >= 0.5) {
      ...
      if (!no_dir) {
        if (str_detect(out_df$Direction[[row]],"\\+"))
          out_df$Direction[[row]] = str_replace_all(out_df$Direction[[row]],"\\+","\\-")
        else
          out_df$Direction[[row]] = str_replace_all(out_df$Direction[[row]],"\\-","\\+")
      }
    }
...
make_table_lead_loci = function(dataset,in_df,maf_cut = 0.01) {
  ...
  if (dataset == "NIA_NIMH") {
    out_df = in_df %>% arrange(CHR,POS) %>%
      rename(MAF=NIA_NIMH_META_MAF,Direction=NIA_NIMH_DIRECTION) %>%
      select(ID,CHR,Rsid,Gene,MAF,EA,Direction,NIA_P,NIMH_P,NIA_NIMH_META_P,UKB_AOU_META_P,NewOld)
    out_df = make_all_minor_alleles(out_df)
    out_df = get_rsids_only(out_df)
    
    # Manual check. Somehow was not marked in meta-analysis when present, so perhaps single error
    out_df %<>% mutate(Direction = ifelse(ID == "19-44895007-C-T","++",Direction))
  }else if (dataset == "UKB_AOU") {
    out_df = in_df %>% arrange(CHR,POS) %>%
      rename(MAF=UKB_AOU_META_MAF,Direction=UKB_AOU_DIRECTION) %>%
      select(ID,CHR,Rsid,Gene,MAF,EA,Direction,NIA_NIMH_META_P,UKB_P,AOU_P,UKB_AOU_META_P,NewOld,AOU_LESS20)
    out_df = make_all_minor_alleles(out_df)
...
make_table_cell_expression = function(df,col,clean_cell_columns,include_signs,only_new,enhancer_role) {
  ...
  out_df = make_all_minor_alleles(out_df,no_dir = T)
  out_df = get_rsids_only(out_df) %>% mutate(MAF = round(MAF,3))
...
make_table_cell_expression_bypath = function(df,coh_col,clean_cell_columns,include_signs=T,only_new,
  ...
  # Flip alleles and plug in rsids
  filt_df = make_all_minor_alleles(filt_df,no_dir = T)
  filt_df = get_rsids_only(filt_df) %>% mutate(MAF = round(MAF,3)) %>%
...
prep_data_for_separate_loci_allcohorts = function(gw) {
  for (coh in c("NIA","NIMH","NIA_NIMH_META","UKB","AOU","UKB_AOU_META","NIA_NIMH_UKB_AOU_META")) {
...
get_multi_nominal_loci = function(df,get_clin_nom,get_direction) {
  ...
  for (coh in c("NIA","NIMH","NIA_NIMH_META","UKB","AOU","UKB_AOU_META","NIA_NIMH_UKB_AOU_META")) {
    ...
    if (get_direction) {
      ...
      }else if (coh == "NIA_NIMH_UKB_AOU_META") {
        rows_to_remove = numeric()
        for (row in 1:nrow(out_df[[coh]])) {
          curr_row = out_df[[coh]][row,]
          sign_str = ""
          if (!is.na(curr_row$NIA_P) & curr_row$NIA_P <= 0.05) sign_str = paste0(sign_str,str_sub(curr_row$NIA_NIMH_UKB_AOU_DIRECTION,1,1))
          if (!is.na(curr_row$NIMH_P) & curr_row$NIMH_P <= 0.05) sign_str = paste0(sign_str,str_sub(curr_row$NIA_NIMH_UKB_AOU_DIRECTION,2,2))
          if (!is.na(curr_row$UKB_P) & curr_row$UKB_P <= 0.05) sign_str = paste0(sign_str,str_sub(curr_row$NIA_NIMH_UKB_AOU_DIRECTION,3,3))
          if (!is.na(curr_row$AOU_P) & curr_row$AOU_P <= 0.05) sign_str = paste0(sign_str,str_sub(curr_row$NIA_NIMH_UKB_AOU_DIRECTION,4,4))
          
          if (str_detect(sign_str,"\\+") & str_detect(sign_str,'\\-')) rows_to_remove %<>% append(row)
        }
        if (length(rows_to_remove) > 0) out_df[[coh]] = out_df[[coh]][-rows_to_remove,]
      }
    }
...
get_nominal_by_gw_sig = function(get_for_intersect) {
  ...
  if (get_for_intersect) {
    ...
    meta_nia_nimh = vroom("final_meta_results/meta_niagads_nimh_chrposrefalt_gw_sig.TBL") %>%
    ...
    meta_aou_ukb = vroom("final_meta_results/meta_ukb_aou_chrposrefalt_gw_sig.TBL") %>%
      rename(ID=MarkerName,Pval = `P-value`) %>% 
      mutate(ID = glue("{CHR}-{POS}-{toupper(Allele2)}-{toupper(Allele1)}")) %>%
      mutate(CHRPOS = glue("{CHR}-{POS}")) %>% 
      mutate(IDrev = glue("{CHR}-{POS}-{toupper(Allele1)}-{toupper(Allele2)}")) %>% select(ID,IDrev,CHRPOS,Pval)
    
    meta_all_study = vroom("final_meta_results/meta_niagads_nimh_ukb_aou_chrposrefalt_gw_sig.TBL",show_col_types = F) %>%
      rename(ID=MarkerName,Pval = `P-value`) %>% 
      mutate(ID = glue("{CHR}-{POS}-{toupper(Allele2)}-{toupper(Allele1)}")) %>%
      mutate(CHRPOS = glue("{CHR}-{POS}")) %>% 
      mutate(IDrev = glue("{CHR}-{POS}-{toupper(Allele1)}-{toupper(Allele2)}")) %>% 
      select(ID,IDrev,CHRPOS,Pval)
    
    for_intersect = nia_gw_sig %>% add_row(meta_nia_nimh) %>% add_row(ukb_gw_sig) %>%
      add_row(aou_gw_sig) %>% add_row(meta_aou_ukb) %>% add_row(meta_all_study)
    ...
    df = vroom("working/gwas_gwsig_for_intersect.txt",show_col_types = F) %>% select(-Pval,-CHRPOS) %>%
      mutate(NIA_P=NA,NIMH_P=NA,NIA_NIMH_META_P=NA,NIA_NIMH_DIRECTION=NA,UKB_P=NA,
             AOU_P=NA,UKB_AOU_META_P=NA,UKB_AOU_DIRECTION=NA,NIA_NIMH_UKB_AOU_META_P=NA,
             NIA_NIMH_UKB_AOU_DIRECTION=NA,NIA_MAF=NA,NIMH_MAF=NA,NIA_NIMH_META_MAF=NA,
             UKB_MAF=NA,AOU_MAF=NA,UKB_AOU_META_MAF=NA,NIA_NIMH_UKB_AOU_META_MAF=NA,
    ...
    nia_nimh_ukb_aou_meta_int = vroom("working/niagads_nimh_ukb_aou_meta_intersects_chrpos.txt",show_col_types = F)
    ...
    # Useful columnns for filtering
    df %<>% separate(ID,into=c("CHR","POS","NEA","EA"),remove=F)

    # Fill in the table
    for (row in 1:nrow(df)) { # check for multiple rows to deal with indels
      ...
      if (curr_row$ID %in% nia_nimh_meta_int$MarkerName | curr_row$IDrev %in% nia_nimh_meta_int$MarkerName) {
        tmp = match_accounting_for_indels(nia_nimh_meta_int %>% rename(ID=MarkerName),curr_row)
        if (nrow(tmp)>0) {
      ...
      if (curr_row$ID %in% ukb_aou_meta_int$MarkerName | curr_row$IDrev %in% ukb_aou_meta_int$MarkerName) {
        tmp = match_accounting_for_indels(ukb_aou_meta_int %>% rename(ID=MarkerName),curr_row)
        if (nrow(tmp)>0) {
      ...
      # NIA NIMH UKB AOU META MATCHING
      if (curr_row$ID %in% nia_nimh_ukb_aou_meta_int$MarkerName | curr_row$IDrev %in% nia_nimh_ukb_aou_meta_int$MarkerName) {
        tmp = match_accounting_for_indels(nia_nimh_ukb_aou_meta_int %>% rename(ID=MarkerName),curr_row)
        if (nrow(tmp)>0) {
          df$NIA_NIMH_UKB_AOU_META_P[[row]] = tmp$`P-value`
          df$NIA_NIMH_UKB_AOU_META_MAF[[row]] = tmp$Freq1
          df$NIA_NIMH_UKB_AOU_DIRECTION[[row]] = tmp$Direction
        }
      }
...
make_df_all_hits_table = function(df) {
  out_df = df %>% filter(is.na(AOU_LESS20) | !AOU_LESS20) %>%
    select(-POS,-NEA,-IDrev,-AOU_LESS20,-NIA_NIMH_UKB_AOU_DIRECTION,-NIA_NIMH_UKB_AOU_META_MAF) %>%
    filter(CHR != 23)
  return(out_df)
}
count_reproduced_loci = function(df,comparison) {
  ...
  if (length(comparison) == 1) {
    for (loc in unique(df$Locus)) {
      if (nrow(df %>% filter(Locus == loc,!!sym(comparison) <= 0.05)) > 0)
        overlap = overlap + 1
    }
  }else{
    for (loc in unique(df$Locus)) {
      if (nrow(df %>% filter(Locus == loc,!!sym(comparison[[1]]) <= 0.05,!!sym(comparison[[2]]) <= 0.05)) > 0)
        overlap = overlap + 1
    }
  }
